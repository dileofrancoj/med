...
export const classifyPotassiumSchema = z.object({
  query: z.object({
    kLevel: z.string({ error: 'Falta parámetro kLevel' })
...
export const rapidCorrectionSchema = z.object({
  body: z.object({
    patient: z.object({
      weight: z.number({ error: 'Los datos del paciente con su peso (weight) son obligatorios' })
...
export const maintenanceSchema = z.object({
  body: z.object({
    patient: z.object({
      ...
      age: z.union([z.number().int().positive(), z.string()]).optional(),
      sex: z.enum(['male', 'female']).optional(),
