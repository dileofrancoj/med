...
export interface ISodiumService {
  classifySodium(naLevel: number): SodiumClassification;
  calculateRapidCorrection(
    request: RapidSodiumCorrectionRequest,
    constants?: ClinicalConstants,
  ): Promise<RapidSodiumCorrectionResponse>;
  calculateSlowCorrection(
    request: SlowSodiumCorrectionRequest,
    constants?: ClinicalConstants,
  ): Promise<SlowSodiumCorrectionResponse>;
}

export class SodiumService implements ISodiumService {
  classifySodium(naLevel: number): SodiumClassification {
    if (naLevel < 135) {
  ...
  async calculateRapidCorrection(
    request: RapidSodiumCorrectionRequest,
